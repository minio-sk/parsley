...
class Parsley
  describe Command do
    ...
    it 'runs command without arguments' do
      Open3.should_receive(:capture2e).with('ls').and_return(['', 0])
      command.run('ls')
    end

    it 'replaces all ? with the appropriate arguments' do
      Open3.should_receive(:capture2e).with('ls /tmp /dev').and_return(['', 0])
    ...
    it 'raises exception if the command fails' do
      Open3.should_receive(:capture2e).and_return(['', 1])
      expect { command.run('ls nodir') }.to raise_error(Parsley::CommandFailed)
    end

    it 'coerces arguments to string' do
      Open3.should_receive(:capture2e).with('ls 200').and_return(['', 0])
      command.run('ls ?', 200)
    end
